...
void Executor::executeInstruction(ExecutionState &state, KInstruction *ki) {
  Instruction *i = ki->inst;
  klee_message("SS_Executor: executeInstruction \n");
  i->print(errs());
  klee_message("\n");
  switch (i->getOpcode()) {
    // Control flow
...
void Executor::run(ExecutionState &initialState) {
  ...
  while (!states.empty() && !haltExecution) {
    ...
    std::map<const std::string *, std::set<unsigned>> covMap = state.coveredLines;
    for (auto it=covMap.begin(); it!=covMap.end(); ++it) {
      //klee_message("Key: %s \n", it->first);
      std::cout << "Filename: ";
    ...
    klee_message("Calling executeInstruction \n");
    executeInstruction(state, ki);
...
void Executor::terminateStateOnExit(ExecutionState &state) {
  if (!OnlyOutputStatesCoveringNew || state.coveredNew || 
      (AlwaysOutputSeeds && seedMap.count(&state)))
    std::cout << "SS_terminateStateOnExit: Exiting State "<<&state<<std::endl;
...
void Executor::runFunctionAsMain(Function *f,
  ...
  klee_message("SS_Executor: runFunctionAsMain\n");
...
bool Executor::getSymbolicSolution(const ExecutionState &state,
  ...
  std::cout << "SS_getSymbolicSolution Values " << std::endl;
  for(int tt=0; tt<values.size(); tt++) { 
    for(auto gg= values[tt].begin(); gg!=values[tt].end(); gg++)
